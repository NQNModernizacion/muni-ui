...
export default function FileInputBase({
  ...
  return (
    <div className={cx("mx-ui", containerClassName ?? "mx-stack")}>
      ...
      <div className={cx("mx-surface", compact ? "mx-file-pad--compact" : "mx-surface-pad")}>
        ...
        {showList && filesArray.length > 0 ? (
          <div className="mx-stack" style={{ marginTop: compact ? ".5rem" : ".75rem" }}>
            {filesArray.map((f) => (
              <div key={`${f.name}-${f.size}-${f.lastModified}`} className="mx-text-sm mx-muted">
                {f.name} <span style={{ opacity: 0.7 }}>({Math.ceil(f.size / 1024)} KB)</span>
              </div>
            ))}
          </div>
        ) : null}
        ...
        {helperText ? (
          <div className="mx-text-xs mx-muted" style={{ marginTop: compact ? ".5rem" : ".75rem" }}>
            {helperText}
          </div>
        ) : null}
      ...
      {preview && firstFile ? (
        <div className={cx("mx-surface", compact ? "mx-file-pad--compact" : "mx-surface-pad")} style={{ marginTop: compact ? ".5rem" : ".75rem" }}>
